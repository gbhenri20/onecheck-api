...
def needs_mfa(user: Usuario) -> bool:
    """Exige MFA no login quando o usuário configurou (qualquer perfil)."""
    return bool(user.mfa_enabled and user.mfa_secret)


def mfa_obrigatorio_nao_configurado(user: Usuario) -> bool:
    """Perfis que devem configurar MFA mas ainda não escanearam o QR."""
