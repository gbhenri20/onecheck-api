...
def login_challenge(body: LoginRequest, db: Session = Depends(get_db)):
    """Valida senha e emite token temporário para MFA (fallback web / apps)."""
    user = db.query(Usuario).filter(Usuario.email == body.email, Usuario.ativo == True).first()
    if not user or not verify_password(body.senha, user.senha_hash):
        return fail("Credenciais inválidas")
    if not needs_mfa(user):
        return fail("MFA não está configurado para esta conta")
    temp = create_temp_token(user.id)
    return ok({
        "mfa_required": True,
        "temp_token": temp,
